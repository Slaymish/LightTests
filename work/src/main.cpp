...
int main() {
  ...
  GLFWwindow *window =
      glfwCreateWindow(800, 600, "Lighting Tests", nullptr, nullptr);
  if (!window) {
  ...
  cout << "Using GLFW " << glfwMajor << "." << glfwMinor << "." << glfwRevision
       << endl;
...
void APIENTRY debugCallback(GLenum source, GLenum type, GLuint id,
  ...
  cerr << "GL [" << getStringForSource(source) << "] " << getStringForType(type)
       << ' ' << id << " : ";
  cerr << message << " (Severity: " << getStringForSeverity(severity) << ')'
       << endl;
